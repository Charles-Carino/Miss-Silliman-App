...
$finalScore = DB::select(DB::raw("
	...
	from
		(select
				...
				((( table_pre.AverageSpeech * .5 ) + ( table_pre.AverageTalent * .34 ) + ( table_pre.SP * .16 )) * .3) prepageant_thirtypercent,
				(((table_pn.AverageProduction*.10) + (table_pn.AverageThemeWr*.25) + (table_pn.AverageEveGown*.25) + (table_pn.AverageSeqIntrvw*.10) + (table_pn.AverageInitIntrvw*.30))  * .6) pageantNight_sixtypercent,
				((pressLaunch.PL_score * .1) + ((( table_pre.AverageSpeech * .5 ) + ( table_pre.AverageTalent * .34 ) + ( table_pre.SP * .16 )) * .3) + (((table_pn.AverageProduction*.10) + (table_pn.AverageThemeWr*.25) + (table_pn.AverageEveGown*.25) + (table_pn.AverageSeqIntrvw*.10) + (table_pn.AverageInitIntrvw*.30))  * .6)) totalInitial
			from
				(select
					can.id
					,concat(upper(can.lName),', ',can.fName) candidates
...
"));
	...
	return view('maintenance.maintenance',compact('judges','organizers','candidates', 'colleges','prepageants','initScores','prePajFinal', 'reports','pressLaunchScores','initialScores','initialScoreSummary','finalScore','bestSpeaker'));
}
