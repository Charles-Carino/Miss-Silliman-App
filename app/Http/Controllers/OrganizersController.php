...
<<<<<<< HEAD
        $talent = User::where('event','Talent')->pluck('id');
        $speech = User::where('event','Speech')->pluck('id');
        $i = 0;
        // dd($talent->count());
        //
        $reportsTalent = DB::select(DB::raw("select
	t0.id,t0.candidates,t0.SP as 'SP',
	t1.talent as 'judge1',t2.talent as 'judge2',t3.talent as 'judge3',
	(t1.talent+t2.talent+t3.talent)/3 as AverageTalent,
	t4.speech as 'judge4',t5.speech as 'judge5',t6.speech as 'judge6',
	(t4.speech+t5.speech+t6.speech)/3 as AverageSpeech,
	(t0.SP*.16) as 'SP16%',
	((t1.talent+t2.talent+t3.talent)/3)*.34 as 'Talent34%',
	((t4.speech+t5.speech+t6.speech)/3)*.50 as 'Speech50%',
...
/*special project*/
	...
	left join candidates can on can.id=pre.candidate
	left join colleges col on col.id=can.college
	WHERE pre.judge = 20 #replace with judge id ie: 4,5 or 6
	group by candidates
	ORDER BY can.id) t4
	/*end*/
	,
	...
	left join candidates can on can.id=pre.candidate
	left join colleges col on col.id=can.college
	WHERE pre.judge = 23 #replace with judge id ie: 4,5 or 6
	group by candidates
	ORDER BY can.id) t5
	/*end*/
	,
	...
	left join candidates can on can.id=pre.candidate
	left join colleges col on col.id=can.college
	WHERE pre.judge = 26 #replace with judge id ie: 4,5 or 6
	group by candidates
	ORDER BY can.id) t6
	/*end*/
...
>>>>>>> e7674355e0920a4d81f7dad4b3005fccb743fc64
    }
}
